...
class DockerCommandRunner:
    """Small subprocess boundary used by the ECR image publisher."""
...
class PublishSageMakerImage:
    """Find or publish the immutable ECR image used by a training attempt."""
    ...
    def execute(self) -> str:
        ...
        if self.rebuild:
            ...
            tag = f"source-{digest_prefix}-{timestamp}"
        ...
        digest = self._existing_digest(tag, required=True)
        return f"{self.repository_uri}@{digest}"
